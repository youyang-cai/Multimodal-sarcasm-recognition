...
from model.detection_model_de import MUSTARDBiModelTri as MUSTARDBiModelTri_test
from model.detection_model_de import MaskedNLLLoss
...
def train_or_eval_model(model, sarcasm_loss_function,implicit_loss_function,explicit_loss_function, dataloader, epoch, optimizer=None, train=False):
    ...
    for data in dataloader:
        ...
        # 假设 data 是一个字典，包含所有必要的键
        if cuda:
            textf = data['text'].cuda()
            visuf = data['visual'].cuda()
            acouf = data['audio'].cuda()
            qmask = data['speakers'].cuda()  # 这里假设 qmask 对应的是 speakers
            umask = data['umask'].cuda()
            sarcasms_label = data['sarcasm_labels'].cuda()
            implicit_label = data['sentiment_imp'].cuda()
            explicit_label = data['sentiment_exp'].cuda()
        else:
            textf = data['text']
            visuf = data['visual']
            acouf = data['audio']
            qmask = data['speakers']
            umask = data['umask']
            sarcasms_label = data['sarcasm_labels']
            implicit_label = data['sentiment_imp']
            explicit_label = data['sentiment_exp']
...
def run_kfold_cross_validation(model, dataset, args, cuda):
    n_splits = 5
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=12345678)
    results = []

    for fold_idx, (train_index, valid_index) in enumerate(kf.split(np.arange(len(dataset)))):
        ...
        for e in range(args.epochs):
            ...
            print(f"Fold {fold_idx+1}, Epoch {e+1}, Train Loss: {train_loss}, Valid Loss: {valid_loss}")
        results.append((train_loss, valid_loss))

    average_train_loss = np.mean([result[0] for result in results])
    average_valid_loss = np.mean([result[1] for result in results])
    print(f"Averaged train loss: {average_train_loss}, Averaged valid loss: {average_valid_loss}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-kfold', action='store_true', help='Use 5-fold cross-validation')
    ...
    if args.use_kfold:
        run_kfold_cross_validation(model, dataset, args, args.cuda)
    else:
        train_loader, valid_loader, test_loader = get_MUSTARD_loaders('MUSTARD_feature/MUSTARD_extract_feature.pkl',
                                                                      valid=0.2,
                                                                      batch_size=args.batch_size,
                                                                      num_workers=2)

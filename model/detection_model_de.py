...
class DialogueRNNTriCell(nn.Module):
    ...
    def forward(self, Ut, Uv, Ua, qmask, g_hist_t, g_hist_v, g_hist_a, q0_t, q0_v, q0_a, e0_t, e0_v, e0_a, k=1):
        ...
        # print("q0_sel_v shape:", q0_sel_v.shape)

        # 假设 Ut 和 q0_sel_t 是需要连接的两个张量
        if Ut.shape[0] != q0_sel_t.shape[0]:
            # 调整批次大小较小的张量
            if Ut.shape[0] < q0_sel_t.shape[0]:
                # 通过重复扩展Ut来匹配q0_sel_t的批次大小
                Ut = Ut.expand(q0_sel_t.shape[0], -1)
            else:
                # 或者通过切片q0_sel_t来匹配Ut的批次大小
                q0_sel_t = q0_sel_t[:Ut.shape[0], :]
        # 为Uv添加一个批次维度，使其形状变为[1, 100]
        Uv = Uv.unsqueeze(0)

        # 扩展Uv以匹配q0_sel_v的批次大小，即2
        Uv = Uv.expand(q0_sel_v.shape[0], -1)  # 使Uv形状变为[2, 100]

        Ua = Ua.unsqueeze(0)

        # 扩展Ua以匹配q0_sel_a的批次大小，即2
        Ua = Ua.expand(q0_sel_a.shape[0], -1)  # 使Ua形状变为[2, 100]
